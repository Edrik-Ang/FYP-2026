...
class SteamAuthURLAPIView(APIView):
    """
    GET /api/integrations/steam/auth-url/
    Returns the Steam OpenID redirect URL. Useful for inspecting what gets
    built, but completing the actual login still requires opening this URL
    in a real, already-authenticated browser session -- the session set here
    and the session Steam's callback resumes must be the same one, which
    Postman's separate cookie jar can't provide.
    """
